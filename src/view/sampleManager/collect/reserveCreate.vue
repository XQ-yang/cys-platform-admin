...
import { createSample, getAppointmentById } from '@/api/collect.js'
...
export default {
  ...
  methods: {
    ...
    getReserveInformation() {
      ...
      getAppointmentById(this.appointmentId).then(res => {
        // 填充项目类型下拉框
        this.fillProjectType(res.data.appointment.sampleType)
        // 加载样本信息
        this.sampleTemp.appointmentId = this.appointmentId
        this.sampleTemp.sampleType = res.data.appointment.sampleType
        this.sampleTemp.projectType = res.data.appointment.projectType
        this.sampleTemp.customerName = res.data.appointment.customerName
        this.sampleTemp.customerPhone = res.data.appointment.customerPhone
        this.sampleTemp.collectHospita = res.data.appointment.collectHospita
        this.sampleTemp.collectTime = res.data.appointment.collectTime
        this.getUserInfo()
        this.sampleTemp.collectTime = new Date()
        // 加载顾客信息
    ...
    fillCustomerTemp(customer) {
      if (customer && customer.id) {
        ...
        this.sampleTemp.customerId = customer.id
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'reserveCreate' && this.appointmentId !== this.$route.query.appointmentId) {
        this.appointmentId = this.$route.query.appointmentId
      }
    },
    appointmentId() {
