        ...
        <Row :gutter="48">
          <Col span="8">
            <Form-item label="血型" prop="customerBloodType">
              <Select v-model="contractTemp.customerBloodType" placeholder clearable disabled>
...
export default {
  ...
  methods: {
      ...
      if (contractId) {
        ...
        getEditData(contractId)
          .then(res => {
            ...
            let projectType = res.data.contract.projectType
            getProjectTypeBySampleType(res.data.contract.sampleType)
              .then(res => {
                this.projectTypeList = res.data
                this.contractTemp.projectType = projectType
              })
              .catch(error => {
    ...
    changeSampleType(sampleType) {
      let projectType = this.contractTemp.projectType
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'editContract' && this.contractTemp.id !== this.$route.query.contractId) {
        // 编辑处化数据
        this.initData()
      }
    }
