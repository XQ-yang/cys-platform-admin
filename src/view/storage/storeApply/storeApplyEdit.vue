...
<script>
import { getCustomerBySampleCode, getStoreApplyById, editStoreApply } from '@/api/storeApply'
...
export default {
  ...
  methods: {
    ...
    getStoreApplyById() {
      getStoreApplyById(this.storeApplyTemp.id).then(res => {
    ...
    submit() {
      ...
      this.$refs['storeApplyForm'].validate(valid => {
        ...
        if (valid) {
          ...
          if (this.validType) {
            editStoreApply(this.storeApplyTemp).then(res => {
              this.changeLoading()
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'storeapplyEdit' && this.storeApplyTemp.id !== this.$route.query.id) {
