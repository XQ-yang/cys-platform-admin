<template>
  <div>
   ...
   <Form ref="outApplyUpdateForm" :model="outApplyTemp" :rules="rules" :loading="outApplyUpdateFormLoading" :label-width="130" >
      <Card title="使用人信息">
           <Row :gutter="48">
              <Col span="8">
            ...
            <Col span="8">
                <Form-item label="出库数量" prop="outQuantity" >
                  <Input type="text" v-model="outApplyTemp.outQuantity" :maxlength="11"></Input>
                </Form-item>
              </Col>
             <Col span="8">
            </Col>
            ...
            <Row :gutter="48">
              ...
              <Col span="8">
                <Form-item label="出库申请时间" prop="outStoreApplyTime" >
                  <DatePicker type="datetime" :value="outApplyTemp.outStoreApplyTime" @on-change="outApplyTemp.outStoreApplyTime=$event"  :options="maxDate"  format="yyyy-MM-dd HH:mm" placeholder="请选择" style="width:100%;"></DatePicker>
                </Form-item>
              </Col>
              <Col span="8">
        ...
        <!--弹出层（转入培养）-->
          ...
          <Form ref="cultureForm" :model="cultureTemp" :rules="cultureRules" :label-width="90" style="padding-right:20px;">
              ...
              <Row>
                ...
                <Col span="12">
                  <Form-item label="项目类型" prop="projectType">
                    <Select v-model="cultureTemp.projectType" placeholder="请选择"  clearable disabled>
              ...
              <Row>
                <Col span="12">
                  <Form-item label="接收时间" prop="receiveTime" >
                    <DatePicker type="datetime" v-model="cultureTemp.receiveTime" :options="maxDate"  format="yyyy-MM-dd HH:mm" placeholder="请选择" disabled></DatePicker>
                  </Form-item>
                </Col>
                <Col span="12">
                  <Form-item label="接收人" prop="receiverId">
                    <Select v-model="cultureTemp.receiverId" :label-in-value="true" placeholder="请选择" filterable clearable disabled>
...
import { updateOutApply, getOutApplyInfo } from '@/api/outApply.js'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      // 细胞出库申请信息
      outApplyUpdateFormLoading: true,
  ...
  methods: {
    ...
    getOutStoreInfo() {
      ...
      getOutApplyInfo(this.cellOutStoreId).then(res => {
        getUnDeleteProjectTypeBySampleType(res.data.sample.sampleType).then(res => {
          // 填充项目类型列表数据
    ...
    // 提交表单，保存运输信息
    submit() {
      this.$refs['outApplyUpdateForm'].validate(valid => {
        if (!valid) {
        ...
        updateOutApply(this.outApplyTemp).then(res => {
          this.changeLoading()
    ...
    // 重置loading状态 防止重复提交
    changeLoading() {
      this.outApplyUpdateFormLoading = false
      this.$nextTick(() => {
        this.outApplyUpdateFormLoading = true
      })
    },
    // 关闭
    close() {
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'outStoreApplyUpdate' && this.cellOutStoreId !== this.$route.query.cellOutStoreId) {
        this.cellOutStoreId = this.$route.query.cellOutStoreId
      }
    },
    cellOutStoreId() {
