  ...
  <div>
   ...
   <!--弹出层（转入培养）-->
    ...
    <Form ref="cultureForm" :model="cultureTemp" :label-width="90" style="padding-right:20px;">
     ...
     <!--库存出库信息-->
  <Form ref="outStockForm" :model="outStockTemp" :loading="outStockFormLoading" :label-width="130" >
        <Card title="出库信息填写">
          <!--细胞位置信息及细胞信息-->
...
export default {
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'outStoreOfStockView' && this.cellOutStoreId !== this.$route.query.cellOutStoreId) {
        this.cellOutStoreId = this.$route.query.cellOutStoreId
      }
    },
    cellOutStoreId() {
