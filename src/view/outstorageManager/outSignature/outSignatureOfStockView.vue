<template>
  ...
  <Form ref="outReviewForm" :model="outReviewTemp" :label-width="130" >
    ...
    <!--签字确认信息-->
    <Form ref="signatureaForm" :model="signatureTemp" :label-width="130" >
        <Card title="签字确认信息">
            <Row :gutter="48">
              <Col span="24">
                <Form-item label="签字文件">
                  <Alert v-for="item in signatureFile" :key="item.id"><a target="_blank" :href="item.mediaPath">{{item.originalMediaName}}</a></Alert>
                </Form-item>
              </Col>
            </Row>
            <Row :gutter="48">
               <Col span="12">
                  <Form-item label="上传日期" prop="uploader_time" >
                    <DatePicker type="datetime" v-model="signatureTemp.uploaderTime"  format="yyyy-MM-dd HH:mm" placeholder="" disabled></DatePicker>
                  </Form-item>
                </Col>
                <Col span="12">
...
import { getSignatureViewInfo } from '@/api/outSignature.js'
export default {
  ...
  methods: {
    ...
    // 加载出库申请信息
    getSignatureViewInfo() {
      this.signatureId = this.$route.query.signatureId
      getSignatureViewInfo(this.signatureId).then(res => {
        getProjectTypeBySampleType(res.data.sample.sampleType).then(res => {
          ...
          this.signatureTemp = res.data.signature
          this.signatureFile = res.data.signatureFile
        }).catch(error => {
          this.$Message.error(error.msg)
        })
      }).catch(error => {
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'outSignatureOfStockView' && this.signatureId !== this.$route.query.signatureId) {
        this.signatureId = this.$route.query.signatureId
      }
    },
    signatureId() {
