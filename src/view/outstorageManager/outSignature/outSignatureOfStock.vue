<template>
            ...
            <Row :gutter="48" style="margin-top:20px;">
              <Col span="8">
                <Form-item label="出库时间" prop="outStoreTime" >
                  <DatePicker type="datetime" v-model="outStockTemp.outStoreTime" :options="maxDate"  format="yyyy-MM-dd HH:mm" placeholder="" disabled></DatePicker>
                </Form-item>
              </Col>
              <Col span="8">
  ...
  <Form ref="outReviewForm" :model="outReviewTemp" :label-width="130" >
   ...
   <!--签字确认信息-->
  <Form ref="signatureForm" :model="signatureTemp" :rules="rules" :loading="signatureLoading" :label-width="130" >
        <Card title="">
          <Row :gutter="48">
            <Col span="24">
              <Form-item label="上传签字文件" prop="signatureFile" class="ivu-form-item-required">
                <Upload
                ref="upload"
                :action="actionUrl"
                :headers="headers"
           ...
           <Row :gutter="48">
            <Col span="8">
                  <Form-item label="上传日期" prop="uploaderTime" >
                    <DatePicker type="datetime" v-model="signatureTemp.uploaderTime" :options="maxDate"  format="yyyy-MM-dd HH:mm" placeholder="" style="width:100%;"></DatePicker>
                  </Form-item>
                </Col>
                <Col span="8">
                  <Form-item label="上传人" prop="uploader">
                    <Input type="text" v-model="signatureTemp.uploader" disabled></Input>
                  </Form-item>
                </Col>
          </Row>
        </Card>
...
import { addSignature } from '@/api/outSignature.js'
export default {
  name: 'outSignatureOfStock',
  data() {
    const validateSignatureFile = (rule, value, callback) => {
      if (!this.$refs.upload.fileList || this.$refs.upload.fileList.length <= 0) {
        callback(new Error('请上传至少一张签字图片'))
      } else {
        callback()
      }
    }
    return {
      ...
      signatureTemp: {
        ...
        uploader: '',
        signatureFile: '',
        uploaderTime: new Date()
      },
      actionUrl: this.$baseUrl + '/media/upload',
      headers: { 'Authorization': getToken() },
      rules: {
        signatureFile: [
          { validator: validateSignatureFile, trigger: 'change' }
        ],
        uploaderTime: [
  ...
  methods: {
    ...
    getUserInfo() {
      getUserInfo().then(res => {
        this.signatureTemp.uploader = res.data.realName
        this.signatureTemp.uploaderId = res.data.id
      }).catch(error => {
        this.$Message.error(error.msg)
      })
    },
    ...
    getOutReviewViewInfo() {
      ...
      getOutReviewViewInfo(this.reviewId).then(res => {
        getProjectTypeBySampleType(res.data.sample.sampleType).then(res => {
          ...
          // 签字确认信息
          this.signatureTemp.reviewId = this.reviewId
          this.signatureTemp.sampleId = res.data.sample.id
          this.signatureTemp.cellOutStoreId = res.data.outstorage.id
        }).catch(error => {
          this.$Message.error(error.msg)
        })
      }).catch(error => {
    ...
    submit() {
      this.$refs['signatureForm'].validate(valid => {
        ...
        this.signatureTemp.signatureFile = this.$refs.upload.fileList.map(file => file.id)
        addSignature(this.signatureTemp).then(res => {
          this.changeLoading()
  ...
  watch: {
    $route(to, from) {
      if (to.name === 'outSignatureOfStock' && this.reviewId !== this.$route.query.reviewId) {
        this.reviewId = this.$route.query.reviewId
      }
    },
    reviewId() {
