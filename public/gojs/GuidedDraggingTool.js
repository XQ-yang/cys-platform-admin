  ...
  if (bestPart !== null) {
    ...
    if (bestSpot === go.Spot.Center) {
      if (snap) {
        ...
        part.move(new go.Point(objBounds.x, bestPoint.y - objBounds.height / 2), true);
      ...
      if (snap) {
        part.move(new go.Point(objBounds.x, bestPoint.y), true);
        this.invalidateLinks(part);
      }
      if (guideline) {
      ...
      if (snap) {
        part.move(new go.Point(objBounds.x, bestPoint.y - objBounds.height), true);
        this.invalidateLinks(part);
      }
      if (guideline) {
...
GuidedDraggingTool.prototype.showVerticalMatches = function(part, guideline, snap) {
  ...
  if (bestPart !== null) {
    ...
    if (bestSpot === go.Spot.Center) {
      if (snap) {
        // call Part.move in order to automatically move member Parts of Groups
        part.move(new go.Point(bestPoint.x - objBounds.width / 2, objBounds.y), true);
        this.invalidateLinks(part);
      }
      if (guideline) {
      ...
      if (snap) {
        part.move(new go.Point(bestPoint.x, objBounds.y), true);
        this.invalidateLinks(part);
      }
      if (guideline) {
      ...
      if (snap) {
        part.move(new go.Point(bestPoint.x - objBounds.width, objBounds.y), true);
        this.invalidateLinks(part);
      }
      if (guideline) {
